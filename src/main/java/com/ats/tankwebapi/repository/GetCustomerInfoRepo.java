...
public interface GetCustomerInfoRepo extends JpaRepository<GetCustomerInfo, Integer>{
	...
	@Query(value = " SELECT  c.*,p.payment_date,p.cost_rs,w.work_date,w.total_amt ,sum(p.cost_rs) ,sum(w.total_amt)"
			+ "FROM t_work w, m_customer c, t_payment p "
			+ "WHERE  c.customer_id =:custId and c.del_status=1 and w.customer_id=c.customer_id and c.customer_id=p.customer_id group by c.customer_id", nativeQuery = true)
